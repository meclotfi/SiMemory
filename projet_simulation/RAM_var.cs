...
namespace ConsoleApp
{
	public class RAM_var : RAM
	{
        ...
        public int Next_fit(int procees_taille)
        {
            ...
            try { return list_rep.IndexOf(list_rep.Find(w => ((w.G_vide() < 0) && (w.Get_taille() >= procees_taille) && (list_rep[next_fit].Get_adr()<=w.Get_adr()))));}
        ...
        public int allocation_process(int prt, int process_id, int process_taille)
		{
			...
			if (nr == 0)
			{
				list_rep[prt].Set_vide(process_id);
				return 0;
			}
			else
			{
		...
		public int Liberation_part(int prt)
		{
			...
			list_rep[prt].Set_vide(-1);
			
			if (prt > 0) prec = list_rep[prt - 1].G_vide();
			else prec = 0;
			if (prt < list_rep.Count) { if (prt + 1 < list_rep.Count) suiv = list_rep[prt + 1].G_vide(); }
			else suiv = 0;
           if ((prec== -1) && (suiv == -1))
			{
